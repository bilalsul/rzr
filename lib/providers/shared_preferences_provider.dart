...
class Prefs extends ChangeNotifier {
  ...
  String get currentProjectPath {
    return prefs.getString('current_project_path') ??
        prefs.getString('app_last_opened_project') ??
        '';
  }
  ...
  final Map<String, Color> _colorNameMap = (() {
    ...
    for (
      var i = 0;
      i < swatchNames.length && i < Colors.primaries.length;
      i++
    ) {
  ...
  Color get secondaryColor {
    ...
    if (stored != null)
      return _colorFromName(stored, fallback: Colors.primaries[2]);
  ...
  Future<void> saveCurrentOpenFile(
    String projectId,
    String filePath,
    String content,
  ) async {
